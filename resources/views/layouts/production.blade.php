...
<head>
    ...
    <meta http-equiv="Content-Security-Policy" content="default-src 'self' https://freegeoip.net/json/ https://maps.google.com https://maps.gstatic.com https://maps.googleapis.com https://staging.ticketbat.com https://www.ticketbat.com;
                          img-src *  'self' blob: http://staging.ticketbat.com {{env('IMAGE_URL_OLDTB_SERVER')}} {{env('IMAGE_URL_AMAZON_SERVER')}} https://d3ofbylanic3d6.cloudfront.net https://s3-us-west-2.amazonaws.com;
    ...
    <meta @if(!empty($ua_conversion_code)) content="{{json_encode($ua_conversion_code,true)}}" @else content="[]"
          @endif name="ua-conversion_code"/>
    <meta @if(!empty($analytics)) content="{{json_encode($analytics,true)}}" @else content="[]"
          @endif name="analytics"/>
...
<div class="page-header navbar navbar-inverse navbar-fixed-top" style="background-color:black!important">
    ...
    <div class="page-header-inner container-fluid fixed-panel">
        <!-- BEGIN LOGO -->
        <div class="page-logo">
            <a @if(!empty(Session::get('slug',null))) href="/event/{{Session::get('slug')}}"
               @else href="{{route('index')}}" @endif title="Go to home page">
        ...
        <div class="collapse navbar-collapse" style="background:#000">
            <ul class="nav navbar-nav @if(!empty(Session::get('funnel',null))) hidden @endif">
                <li @if( url()->current() == route('index') ) class="active" @endif>
                    <a @if(!empty(Session::get('slug',null))) href="/event/{{Session::get('slug')}}" @else href="/home"
                       @endif class="menu_nav" title="Go to home page">
            ...
            <ul class="nav navbar-nav navbar-right">
                @if(!Auth::check())
                    <li>
                        <a data-toggle="modal" href="#modal_register" class="menu_nav"
                ...
                @else
                    <li class="dropdown @if(preg_match('/\/user/',url()->current())) active @endif">
                        <a href="#" class="dropdown-toggle menu_nav" data-toggle="dropdown"
                           title="View your user's options">
                ...
                <li class="dropdown-notification @if(preg_match('/\/shoppingcart/',url()->current())) active @endif">
                    <a href="/shoppingcart/viewcart" class="menu_nav" title="View/pay you items in the shopping cart">
                        <i class="icon-basket"></i> Shopping Cart <span class="badge badge-danger"><b
                                    id="shoppingcart_qty_items" style="font-size:14px">Loading</b></span>
                    </a>
                </li>
            </ul>
        </div>
    ...
    @if(in_array(Auth::user()->user_type_id,explode(',',env('SELLER_OPTION_USER_TYPE'))))
        <div id="timerClockPanel" class="hidden"></div>
    @else
        <div id="timerClockPanel">
            ...
            <a href="/shoppingcart/viewcart" style="font-size:10px;font-weight:bold" class="btn btn-success">checkout
                now</a>
        </div>
    @endif
...
<div class="page-footer fixed-panel">
    <div class="page-footer-inner">
        ...
        <a class="menu_foot" data-toggle="modal" href="#modal_contact_us"
           title="If you have any issues, please, let us know">Contact Us</a>
    </div>
