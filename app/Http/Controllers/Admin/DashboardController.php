        ...
        try {
            ...
            {   
                if(isset($input['start_date']) && $input['start_date'] && isset($input['end_date']) && $input['end_date'])
                {
                    ...
                    (isset($input['order']) && $input['order']=='url')? $order = 'GROUP BY referral_url,s.id' : $order = 'GROUP BY s.id,referral_url';
                    //get all records        
                    $data = DB::select('SELECT COALESCE(SUBSTRING_INDEX(SUBSTRING_INDEX(p.referrer_url, "://", -1),"/", 1), "-Not Registered-") AS referral_url,
                                            s.name as show_name, sum(p.quantity) AS qty_tickets, COUNT(*) AS qty_purchases,
                                        ...
                                        WHERE p.status = "Active" AND p.created BETWEEN ? AND ? AND p.referrer_url IS NOT NULL '.$order,
                            array(date_format(date_create($input['start_date']),'Y-m-d H:i:s'),date_format(date_create($input['end_date']),'Y-m-d H:i:s')));
                }
            }
            //calculate totals
