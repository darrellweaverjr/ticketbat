    ...
    public function buy()
    {
        try {
            ...
            && !empty($info['email']) && !empty($info['card']) && !empty($info['month']) && !empty($info['year']) && !empty($info['cvv']))
            {
                ...
                if($sent)
                    return Util::json(['success'=>true, 'msg'=>'We sent you a receipt by email.<br>You can also see the purchases and the tickets at Profile -> My Purchases.']);
                return Util::json(['success'=>true, 'msg'=>'We could not send you a receipt by email.<br>You can see the purchases and the tickets at Profile -> My Purchases.']);
            }
