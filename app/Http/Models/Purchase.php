        ...
        for ($i=1; $i<=$this->quantity; $i++)
            foreach($ticket_numbers as $tn)
                if(in_array($i,explode(',',$tn->tickets)))
                {
                    ...
                    $extra_info = ['show_name'=>$purchase->show_name,'show_time'=>$purchase->show_time,'price_each'=>round($this->price_paid/$this->quantity,2),'id'=>$this->id,'venue_name'=>$purchase->venue_name,'restrictions'=>$purchase->restrictions,'user_id'=>$this->user_id,'ticket_type'=>$purchase->ticket_type_type,'time_alternative'=>$purchase->time_alternative];
    ...
    public static function email_receipts($subject,$receipts,$type_email)
    {
        try {
            if(is_array($receipts) && count($receipts) && is_string($subject) && is_string($type_email))
            {
                ...
                //send email           
                $email = new EmailSG(env('MAIL_REPORT_FROM'), $receipt['customer']->email , $subject);
