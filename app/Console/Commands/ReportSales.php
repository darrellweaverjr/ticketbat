    ...
    public function handle()
    {
        ...
        function calculate_total($elements)
        {
            ...
            foreach ($elements as $e)
                $total = array('t_ticket'=>$total['t_ticket']+$e['qty'], 't_purchases'=>$total['t_purchases']+$e['purchase_count'], 't_gross_revenue'=>$total['t_gross_revenue']+$e['gross_revenue'], 't_processing_fee'=>$total['t_processing_fee']+$e['processing_fee'], 't_commission_percent'=>$total['t_commission_percent']+$e['commission_percent'], 't_net'=>$total['t_net']+$e['net'], 't_commission'=>$total['t_commission']+$e['commission']);
        ...
        function sendEmailReport($data,$send,$date_report,$sqlMain,$sqlFrom)
        {
            ...
            $email->body('sales_report',array('date'=>date_format(date_create($date_report), 'm/d/Y')));
            $email->template('a6e2bc2e-5852-4d14-b8ff-d63e5044fd14');
            //$email->attachment('/tmp/sales_report.pdf');
            if($send == 'admin')
            {
                /*//SALES REFERRER PDF
                $format = 'referrer';
                $purchases = DB::select($sqlMain.$sqlFrom." GROUP BY referrer_url,p.show_time_id, p.ticket_type;");
                $manifest_email_referrer = View::make('command.report_sales', compact('purchases', 'date_report','format'));
                $pdf_ =  PDF::load($manifest_email_referrer->render(), 'A4', 'landscape')->output();
                $pdf_referrer_ = "/tmp/ReportSales_".date('Y-m-d').".pdf";
                $fp_pdf_ = fopen($pdf_referrer_, "w");
                fwrite($fp_pdf_, $pdf_);
                fclose($fp_pdf_);
                PDF::reinit();
                ...
                $csv_path = "/tmp/ReportSales_".date('Y-m-d').".csv";
