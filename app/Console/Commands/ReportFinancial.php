...
class ReportFinancial extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            $summary[] = calc_totals($table1,'Total Tickets');
            $tables[] = ['title'=>'Period: '.date('F j, Y', strtotime($start)).' to '.date('F j, Y', strtotime($end)),'data'=>$table1];
            //get all purchases totals grouped by month
            $table2 = DB::table('purchases')
                        ->join('show_times', 'show_times.id', '=', 'purchases.show_time_id')
                        ->join('shows', 'shows.id', '=', 'show_times.show_id')
                        ->join('venues', 'venues.id', '=', 'shows.venue_id')
                        ->select(DB::raw('venues.id, venues.name,
            ...
            $table3 = DB::table('purchases')
                        ...
                        ->groupBy('venues.id')->orderBy('venues.name')
                        ->distinct()->get()->toArray();
            $summary[] = calc_totals($table3,'Total Tickets YTD');
            $tables[] = ['title'=>'YTD: '.'January 1, '.date('Y',strtotime($start)).' to '.date('F j, Y',strtotime($end)),'data'=>$table3];
            //get all purchases totals YOY same period
            $table4 = DB::table('purchases')
                        ->join('show_times', 'show_times.id', '=', 'purchases.show_time_id')
                        ->join('shows', 'shows.id', '=', 'show_times.show_id')
                        ->join('venues', 'venues.id', '=', 'shows.venue_id')
                        ->select(DB::raw('venues.id, venues.name,
